...
import es.uji.control.sip.preferences.internal.PreferenceManagerSupplier;
...
public class ContributedPreferenceNode extends PreferenceNode {
	 ...
	 */
	public ContributedPreferenceNode(String id, String label, ImageDescriptor imageDescriptor,
		...
		this.nodeQualifier = (nodeQualifier != null) ? nodeQualifier
				: FrameworkUtil.getBundle(pageClass).getSymbolicName();

		this.context = EclipseContextFactory.getServiceContext(FrameworkUtil.getBundle(pageClass).getBundleContext());
	...
	public void createPage() {
		...
		try {
			...
			if (this.logger != null) {
				this.logger.log(LogService.LOG_ERROR, "Error on creating instance of " + this.pageClass.getName(), e);
			}
		}
	}
